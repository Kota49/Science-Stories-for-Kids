...
class Generator extends \yii\gii\generators\model\Generator
{
        ...
        if (! empty($this->moduleName)) {
            if (empty($this->tableName)) {
                ...
                if ($this->useTablePrefix) {
                    ...
                    $this->tableName = $db->tablePrefix;
                }
                $this->tableName .= $this->moduleName . '_';
            }
    ...
    private function generateManyManyRelationsList($table, $fks, $relations)
    {
        ...
        foreach ($fks as $pair) {
            ...
            if (! isset($firstKey[1])) {
                VarDumper::dump($firstKey);
                VarDumper::dump($secondKey);
                exit();
            }
            ...
            foreach ($db->getSchema()->getTableSchemas($schemaName) as $table) {
                ...
                foreach ($table->foreignKeys as $foreignKey => $refs) {
                    ...
                    $relations[$table->fullName][$relationName] = [
                        "return \$this->hasOne($refClassName::className(), $link)->cache();",
                        $refClassName,
                        false,
    ...
    public function generateRules($table)
    {
        ...
        foreach ($table->columns as $column) {
            ...
            if (preg_match('/(name)$/i', $column->name)) {
                $rules[] = "[['" . $column->name . "'], 'app\components\\validators\TNameValidator' ]";
            }
            if (preg_match('/(password)$/i', $column->name)) {
                $rules[] = "[['" . $column->name . "'],'app\components\\validators\TPasswordValidator','length' => 8  ]";
            }
