...
class Generator extends \yii\gii\generators\crud\Generator
{
    ...
    public $adminControllerClass;

    public $baseControllerClass = 'app\components\TController';

    public $adminbaseControllerClass = null;
    ...
    public function beforeValidate()
    {
        ...
        if (strstr($this->modelClass, 'modules')) {
    ...
    {
        $files = [];
        if ($this->enableUserMode) {
            $controllerFile = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->controllerClass, '\\')) . '.php');
            $files[] = new CodeFile($controllerFile, $this->render('controller.php'));
        }
        if ($this->enableAdminMode) {
            $this->baseControllerClass = $this->controllerClass;
            $this->adminbaseControllerClass = 'Base' . StringHelper::basename($this->baseControllerClass);
            $this->adminControllerClass = str_replace('controllers', 'controllers\\admin', $this->controllerClass);
            $controllerFile = Yii::getAlias('@' . str_replace('\\', '/', ltrim($this->adminControllerClass, '\\')) . '.php');
            $files[] = new CodeFile($controllerFile, $this->render('admincontroller.php'));
        }
    ...
    public function generateActiveField($attribute)
    {
        ...
        if ($column->phpType === 'boolean' || $column->dbType == 'tinyint(1)') {
            return "\$form->field(\$model, '$attribute')->checkbox()" . "//\$form->field(\$model, '$attribute')->widget(kartik\widgets\SwitchInput::class,[])";
            ...
            return "\$form->field(\$model, '$attribute')->widget(yii\jui\DatePicker::class,
			[
            ...
            return "\$form->field(\$model, '$attribute')->widget(kartik\widgets\TimePicker::class,[])";
            ...
            return "\$form->field(\$model, '$attribute')->widget(kartik\widgets\Select2::class,['data' => array_merge(['' => ''], \$model->\$attribute),])";
        } else {
