...
use app\modules\smtp\models\search\Unsubscribe as UnsubscribeSearch;
use Yii;
 ...
 */
class UnsubscribeController extends TController
{
    ...
    public function actionIndex()
    {
        ...
        $model->state_id = Unsubscribe::STATE_ACTIVE;
    ...
    /**
     * Clone an existing Unsubscribe model.
     * If update is successful, the browser will be redirected to the 'view' page.
     *
     * @param integer $id
     * @return mixed
    ...
    /**
     * Truncate an existing Unsubscribe model.
    ...
    {
        $query = Unsubscribe::find();
        foreach ($query->each() as $model) {
            $model->delete();
        }
        if ($truncate) {
            Unsubscribe::truncate();
        }
        \Yii::$app->session->setFlash('success', 'Unsubscribe Cleared !!!');
        return $this->redirect([
            'index'
        ]);
    }
    ...
    public function actionDeleteUnsubscribe($email)
    {
        $exist = Unsubscribe::check($email);
      
        if ($exist) {
            $exist->delete();
            \Yii::$app->session->setFlash('info', 'Email Removed from Unsubscribed!');
            return $this->redirect(Yii::$app->request->referrer);
        }
    }

    public function actionSelectUnsubscribe($email)
    {
        if (Unsubscribe::check($email)) {
            \Yii::$app->session->setFlash('info', 'Email Already Unsubscribed !');
            return $this->redirect(Yii::$app->request->referrer);
        }
        Unsubscribe::add($email);
        \Yii::$app->session->setFlash('success', 'Email Added to Unsubscribe !');
        return $this->redirect(Yii::$app->request->referrer);
    }
}
