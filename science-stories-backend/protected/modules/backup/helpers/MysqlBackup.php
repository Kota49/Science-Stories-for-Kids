...
class MysqlBackup
{
    ...
    {
        $str = PHP_EOL . '-- -------------------------------------------' . PHP_EOL . 'SET SQL_QUOTE_SHOW_CREATE = 1;' . PHP_EOL . 'SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;' . PHP_EOL . 'SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;' . PHP_EOL . '-- -------------------------------------------' . PHP_EOL;
        return $str;
    }

    public function botHeading()
    {
        $str = PHP_EOL . '-- -------------------------------------------' . PHP_EOL . 'SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;' . PHP_EOL . 'SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;' . PHP_EOL . '-- -------------------------------------------' . PHP_EOL;
    ...
    public function fullBackup($addcheck = true)
    {
        ...
        if (shell_exec('which mysqldump')) {
            ...
            $cmd = 'mysqldump --no-tablespaces -h ' . $this->host . ' -u ' . $this->username . ' -p' . $this->db_password . ' ' . $this->db_name . '> ' . escapeshellarg($this->getFilePath());
            self::log(":cmd : " . $cmd);
    ...
    private function writeData($dataReader, $tableName)
    {
        ...
        foreach ($dataReader as $data) {
            ...
            for ($i = 0; $i < count($itemValues); $i ++) {
                ...
                if ($itemValues[$i]) {
                    $itemValues[$i] = addslashes($itemValues[$i]);
                }
            }
